...
@CrossOrigin
public interface ChildCrudRestController<T extends BaseModel> {
    ...
    Page<T> get(@PathVariable UUID parentId,@RequestParam(required = false, defaultValue = "0") Integer page, @RequestParam(required = false, defaultValue = "100") Integer size, @RequestParam(required = false) String fields) throws IOException;

    @ApiOperation(value = "Single instance of type")
    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    T get(@PathVariable UUID parentId,@PathVariable UUID id, @RequestParam(required = false) String fields);

    @ApiOperation(value = "Delete instance of type")
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
    void delete(@PathVariable UUID parentId,@PathVariable UUID id, @RequestHeader(name = "Authorization") String jwtToken);

    @ApiOperation(value = "Create instance of type")
    @RequestMapping(method = RequestMethod.POST)
    UUID post(@PathVariable UUID parentId,@RequestBody T body, @RequestHeader(name = "Authorization") String jwtToken);

    @ApiOperation(value = "Create bulk instances of type")
    @RequestMapping( method = RequestMethod.POST, value = "bulk")
    List<UUID> postBulk(@PathVariable UUID parentId,@RequestBody List<T> b, @RequestHeader(name = "Authorization") String jwtToken);
